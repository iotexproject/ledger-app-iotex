...
cmake --build "$BUILDDIR" --target fuzz

# Run the fuzzer on the corpus files
export LLVM_PROFILE_FILE="$BUILDDIR/fuzz.profraw"
"$BUILDDIR/fuzz" "$CORPUSDIR"/*
llvm-profdata merge --sparse "$LLVM_PROFILE_FILE" -o "$BUILDDIR/fuzz.profdata"
llvm-cov show "$BUILDDIR/fuzz" -instr-profile="$BUILDDIR/fuzz.profdata" -show-line-counts-or-regions -output-dir="$HTMLCOVDIR" -format=html
llvm-cov report "$BUILDDIR/fuzz" -instr-profile="$BUILDDIR/fuzz.profdata"
